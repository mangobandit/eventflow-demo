(() => {
  ...
  function guestPracticalAnswer(text) {
    ...
    if (/diet(?:ary)?|allerg(?:y|ies|ic)|coeliac|celiac|gluten|dairy[- ]?free|vegan|vegetarian|halal|kosher|food intolerance/i.test(text)) return "Please let Matt or Cara know about any dietary requirements or allergies.";
    if (/accessib(?:le|ility)|access needs|step[- ]?free|wheelchair|mobility|disabled|disabilit(?:y|ies)|close drop[- ]?off/i.test(text)) return "The venue is wheelchair friendly.";
    ...
    if (/\b(rsvp|check\s*-?\s*in|confirm|confirmation|invite|invitation)\b/i.test(text)) return "Matt and Cara will send your guest check in link a few days before the wedding.";
    return null;
  }
